...
//  });
//
//  const amountDueRightX = width - MARGIN;
//
//  drawText(`Amount Due (${(invoice.currency || "LKR").toUpperCase()})`, amountDueRightX - 150, y, {
//    size: 10.5,
//    bold: true,
//    color: colors.muted,
//  });
...
async function buildInvoicePdf(invoice) {
  ...
  const amountDueRightX = width - MARGIN;
  const amountDueLabel = `Amount Due (${(invoice.currency || "LKR").toUpperCase()})`;
  const amountDueLabelWidth = textWidth(amountDueLabel, 10.5, true);
  ...
  const billBottomY = drawWrappedText(billToText || "-", MARGIN, y, 260, {
    size: 9.5,
    lineHeight: 14,
    color: colors.text,
  });
  ...
  // PAYMENT TERMS + NOTES
  let contentBottom = rowY - 14;

  if (invoice.payment_terms || invoice.notes) {
    contentBottom = ensureSpace(contentBottom, 120);

    if (invoice.payment_terms) {
      drawText("Payment Terms", MARGIN, contentBottom, {
        size: 10.5,
        bold: true,
        color: colors.muted,
      });
    ...
    if (invoice.notes) {
      drawText("Notes", MARGIN, contentBottom, {
        size: 10.5,
        bold: true,
        color: colors.muted,
      });
